...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QPushButton, QLabel, QComboBox, QMessageBox, QFileDialog, QSpinBox

from image_manipulation import saving

class KmeansControlsView(QWidget):
    ...
    def setup_ui(self):
        ...
        self.components_input = QSpinBox()
        self.components_input.setMinimum(1)
        self.components_input.setMaximum(1000)
        self.components_input.setValue(1)
        self.components_input.setFixedHeight(35)
    ...
    def update_components_limit(self):
        ...
        if path := self.image_combo.currentData():
            if metadata := self.parent.parent.image_data.get(path, {}).get('metadata'):
                ...
                self.components_input.setMaximum(bands)
                self.components_input.setValue(min(self.components_input.value(), bands))
    ...
    def execute_function(self):
        ...
        n_components = self.components_input.value()
        ...
        if self.run_callback:
            try:
                self.run_callback(path, mask_path, n_components)
